...
(function ($) {
    $.extend({
    	...
    	table: {
			...
			//lmf 自定义返回多个图片视图的方法
			imageViews: function (value, path, target) {
				var _path = $.common.isEmpty(path) ? '/profile/upload' : path;
				// blank or self
				var _target = $.common.isEmpty(target) ? 'self' : target;
				if ($.common.isNotEmpty(value)) {
					var ivStr="";
					for (var ivNum=0;ivNum<value.length;ivNum++){
						ivStr+=$.common.sprintf("<img class='img-circle img-xs' data-target='%s' src='%s/%s'/>", _target, _path, value[ivNum].path);
					}
					return ivStr;
				} else {
					return $.common.nullToStr(value);
